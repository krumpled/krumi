...
export function fromNullable<T>(item: T | undefined | null): Option<T> {
  return item === undefined || item === null ? { kind: 'none' } : { data: item, kind: 'some' };
}
...
export function mapAsync<T, U>(option: Option<T>, mapper: (inner: T) => Promise<U>): Promise<Option<U>> {
  return option.kind === 'some' ? mapper(option.data).then(some) : Promise.resolve(option);
}
